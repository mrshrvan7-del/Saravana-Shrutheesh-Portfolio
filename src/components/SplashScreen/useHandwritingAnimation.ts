...
import { SignaturePathData } from './generateSignaturePaths';

export function useHandwritingAnimation(
  paths: SignaturePathData[],
  ...
  useAnimationFrame((timestamp) => {
    ...
    const currentStroke = paths[currentIndex];
    const strokeElapsed = timestamp - strokeStartTimeRef.current!;
    
    // curve timing and easing adjustments
    const isCurve = currentStroke.type === 'curve';
    const duration = isCurve ? currentStroke.naturalDuration * 1.15 : currentStroke.naturalDuration;

    let rawProgress = strokeElapsed / duration;
    if (rawProgress > 1) rawProgress = 1;

    // Handwriting Easing Function:
    // t < 0.1 ? 5 * t * t : t > 0.8 ? 1 - 2 * (1-t) * (1-t) : 0.05 + 0.9 * ((t - 0.1) / 0.7)
    const t = rawProgress;
    let easedProgress = 0;
    if (isCurve) {
      // Slightly slower mid-section easing for curves
      easedProgress = t < 0.1
        ? 5 * t * t
        : t > 0.8
        ? 1 - 2 * (1 - t) * (1 - t)
    ...
    if (rawProgress >= 1) {
      if (pathEl) {
        ...
        pathEl.setAttribute('stroke', '#2AADBA'); // Set to dry ink color
      }
      ...
      // Update index state (rare update, max once per stroke, not 60fps)
