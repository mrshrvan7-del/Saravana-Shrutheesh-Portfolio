...
// Y=150 and Size=180 to center "Saravana" on 800x200 canvas
const text = 'Saravana';
const fontSize = 180;
const yOffset = 150;

// Pre-calculate text width to center horizontally on 800px canvas
let totalUnits = 0;
for (let i = 0; i < text.length; i++) {
  const glyph = font.charToGlyph(text[i]);
  totalUnits += glyph.advanceWidth || 0;
}
const textWidth = totalUnits * (fontSize / font.unitsPerEm);
let cursorX = (800 - textWidth) / 2;
